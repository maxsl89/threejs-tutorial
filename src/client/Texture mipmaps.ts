// Mipmapping - это метод рендеринга текстуры, который применяется для каждой текстуры.

// Когда mipmapping включен (по умолчанию), графический процессор будет использовать версии текстуры разных размеров для рендеринга поверхности в зависимости от того, как далеко она находится от камеры.
